...
import {
  concatArrays,
  createError, filterDuplicate, filterKey, getAttrKeys, getDynamicElements, getTextKeys, testRegex
} from "../../../shared/utils";
...
export class Component {
  ...
  render(replaceTags?:boolean, trimHTML?:boolean): void {
    if (typeof this.selector !== "undefined") {
      const setDynamicNodes = (key:string, isAllUpdate:boolean = false) =>{
        ...
        if(this._dynamic.dynamicNodes.length < 2048){
          this._dynamic.dynamicNodes.forEach((e, i)=>{
            ...
            filtredValues.forEach((filtredVal: DynamicTextType)=>{
              ...
              this._dynamic.dynamicNodes[i].dynamicTexts[index] = e.updateText(val,filtredVal,e.texts);
              this._dynamic.dynamicNodes[i].texts = filterDuplicate(concatArrays(this._dynamic.dynamicNodes[i].texts, this._dynamic.dynamicNodes[i].dynamicTexts[index].texts));
              this._dynamic.dynamicNodes[i].texts = this._dynamic.dynamicNodes[i].texts.filter(text=>{
                if(text){
                  return text;
                }
              })
      ...
      const setNode = (el:Element, index:number) =>{
        ...
        const node : NodeType = {
          updateText:(val:any = undefined, updVal:DynamicTextType,texts:TextArrayType) => updateText(el, val, updVal, index,texts),
          updateAttr:(val:any, key:string)=> updateAttributes(el,val,index,attrs,key),
          index,
          attrs,
          texts:[],
          dynamicAttrs,
      ...
      const render = () =>{
        ...
        this._dynamic.dynamicNodes.forEach((e, i)=>{
          e.dynamicTexts.forEach((val, j) => {
            this._dynamic.dynamicNodes[i].dynamicTexts[j] = e.updateText(returnDataValue(this.data,val.key), val, e.texts);
            this._dynamic.dynamicNodes[i].texts = filterDuplicate(concatArrays(this._dynamic.dynamicNodes[i].texts, this._dynamic.dynamicNodes[i].dynamicTexts[j].texts));
            this._dynamic.dynamicNodes[i].texts = this._dynamic.dynamicNodes[i].texts.filter(text=>{
              if(text){
                return text;
              }
          ...
          if(Object.keys(e.attrs).length){
            e.dynamicAttrs.forEach(keyAttr=>{
              e.attrs = e.updateAttr(returnDataValue(this.data,keyAttr), keyAttr);
            })
          }
        });
        this._dynamic.dynamicNodes = [];
      ...
      const updateFunction = (name: string, key: string, isRender:boolean = false) => {
        const defaultData =
      ...
      const renderScriptsAndStyles = (e:Element | null, start:StartType) =>{
      ...
      const renderDynamicArray = (e:Element | null , index:number) =>{
        if(e){
      ...
      document.querySelectorAll(condition?`template[data-cample=${this.selector}]`: this.selector).forEach((e, index) => {
        renderFunctionsData(this.data, updateFunction, true);
        renderScriptsAndStyles(e, "beforeLoad");
        const functionsArray:FunctionsArray = [];
        functionsArray.push((el:Element | null)=>renderDynamicArray(el,index));
        functionsArray.push((el:Element | null)=>renderScriptsAndStyles(el, "afterLoad"));
        renderHTML(e,this.template,this.replaceTag,replaceTags,functionsArray,"component",trim);
      });
    } else {
